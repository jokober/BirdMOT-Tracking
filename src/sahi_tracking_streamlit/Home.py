...
if st.session_state['dataset_path'] and st.session_state['experiment_path'] and st.session_state[
    'prediction_params_path']:
    if st.session_state['experiment_path'] != "All":
        ...
        state_persistance, dataset, predictions_result, evaluation_results_list = load_data(tracking_dataset_dict,
                                                                                            sahi_predictions_params_dict,
                                                                                            tracking_experiment_dict)
    if st.session_state['experiment_path'] == "All":
    ...
    for eval in evaluation_results_list:
        ...
        tracking_result = state_persistance.load_data('tracking_results', eval['tracking_results_hash'])[
            'tracking_results']
        ...
        # Add fps for detection.
        prediction_result = state_persistance.load_data('predictions_results', eval['predictions_hash'])
        if 'fps' in prediction_result:
            eval['det_fps'] = prediction_result['fps']
        else:
            eval['det_fps'] = None
    ...
    [state_persistance.load_data("evaluation_results", eval_hash)["path"] for eval_hash in
     df_select["eval_results_hash"]]

    st.markdown("# 2 Tracker Results")
    df_select = df_select.sort_values(by=['HOTA'], ignore_index=True, ascending=False)
    df_select.index += 1
    st.dataframe(df_select, column_order=['TrackerName', 'HOTA', 'DetA', 'AssA', 'LocA', 'fps', 'det_fps'])

    selected_eval_results = [state_persistance.load_data('evaluation_results', eval_hash) for eval_hash in
                             df_select['eval_results_hash'].to_list()]
    ...
    with st.expander("AssA vs DetA (HOTA)"):
        st.markdown(
            ...
            **DetA:** Percentage of aligning detections.
            """)
        st.pyplot(create_comparison_plot(selected_trackeval_data, "/media/data/BirdMOT/local_data_tracking/",
    ...
    with st.expander("AssPr vs AssRe (AssA)"):
        st.markdown(
            ...
            **AssPr:** Measures how well tracks can avoid merging multiple objects together into a single track
            """)
        st.pyplot(create_comparison_plot(selected_trackeval_data, "/media/data/BirdMOT/local_data_tracking/",
                                         *['AssPr', 'AssRe', 'HOTA', 'AssA', 'jaccard']), use_container_width=False)
    ...
    with st.expander("DetPr vs DetRe (DetA)"):
        st.markdown(
            ...
            **DetRe:** Measures how well a tracker finds all the ground-truth detections
            """)
        st.pyplot(create_comparison_plot(selected_trackeval_data, "/media/data/BirdMOT/local_data_tracking/",
                                         *['DetPr', 'DetRe', 'HOTA', 'DetA', 'jaccard']), use_container_width=False)
    ...
    with st.expander("HOTA vs LocA"):
        st.markdown(
            ...
            **:**
            """)
        st.pyplot(create_comparison_plot(selected_trackeval_data, "/media/data/BirdMOT/local_data_tracking/",
                                         *['HOTA', 'LocA', 'HOTA', None, None]), use_container_width=False)

    for plt in plot_compare_trackers(trackeval_data, "pedestrian",
                                     output_folder="/media/data/BirdMOT/local_data_tracking/", plots_list=None):
        st.pyplot(plt)

    detections_res = state_persistance.load_data('predictions_results', track_res['predictions_hash'])
    df = trackeval_to_pandas(
        evaluation_results_list[0]['evaluation_results'][0]['MotChallenge2DBox']['default_tracker'])
    ...
    for tracking_hash in unique_tracking_hashes:
        tracking_param_set.update(
            get_tracking_experiment_conf(tracking_hash)['tracking_experiment']['tracker_config'].keys())
    ...
    if len(tracking_param_options) > 0:
        for param in tracking_param_options:
            for tracking_hash in unique_tracking_hashes:
                df.loc[df['tracking_results_hash'] == tracking_hash, param] = \
                    get_tracking_experiment_conf(tracking_hash)['tracking_experiment']['tracker_config'].get(param)
    ...
    for tracking_hash in unique_tracking_hashes:
        if 'filter' in get_tracking_experiment_conf(tracking_hash)['tracking_experiment']:
            filter_conf_df = pd.json_normalize(
                get_tracking_experiment_conf(tracking_hash)['tracking_experiment']['filter'], sep='_')
    ...
    metrics_options = st.multiselect(
        ...
        ['DetA___5', 'AssA___5', 'DetRe___5', 'DetPr___5', 'AssRe___5', 'AssPr___5', 'HOTA___95'] + METRICS)
    ...
    if len(filtered_seq_tracking_hash) == 0:
        ...
        viz_obj = SequenceVizualizer.from_det_gt_pred(df_seq_select['seq_name'].to_list()[0], dataset_res,
                                                      detections_res,
        ...
        with cols[3]:
            st.button("Last", on_click=last, use_container_width=True)
        with cols[2]:
            st.text(str(st.session_state.counter) + '/' + str(viz_obj.total_frames))
        with cols[1]:
            st.button("First", on_click=first, use_container_width=True)
        with cols[0]:
        ...
        with container.container():
            if st.session_state.counter == -1:
                st.session_state.counter = viz_obj.total_frames
