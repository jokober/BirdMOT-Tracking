...
class SequenceVizualizer:
    def __init__(self, mot_img_path: Path, gt_path: Path, det_path: Path, track_res_path: Path,
                 show_detections: bool = True, show_gt: bool = True, show_track_res: bool = True, show_all_tracks=False):
    ...
    def annotate_compl_tracks_on_frame(self, img):
        ...
        if self.show_track_res:
            for line in self.track_res:
                img = cv.rectangle(img, (int(line[2]), int(line[3])), (int(line[4]), int(line[5])), self.colors[int(line[1])]*np.array((255,255,255)),
                                   3)  # top-left corner and bottom-right corner of rectangle
        if self.show_gt:
            for line in self.gt:
    ...
    def get_cropped_miniature_objects(self):
        ...
        for instance_id in instance_ids:
            ...
            for img_ann in img_annotations:
                ...
                cropped_image = img[int(center[1]-crop_size/2):int(center[1]+crop_size/2), int(center[0]-crop_size/2): int(center[0]+crop_size/2)]
    ...
    def create_frame_gallery(self, max_rows: int, max_cols: int):
        ...
        for instance_id, imgs_list in cropped_imgs_by_id.items():
            ...
            print(frames_total)
            print(max_cols)
            nrows = min(max_rows, int(math.ceil(frames_total/max_cols)))
    ...
    def from_det_gt_pred(cls, seq_name: str, dataset_res: dict, detections_res: dict, track_res: dict,
                         show_detections: bool = True, show_gt: bool = True, show_track_res: bool = True, show_all_tracks: bool = False):
        mot_img_path = [seq for seq in dataset_res['dataset']['sequences'] if seq['name'] == seq_name][0]['mot_path'] / 'img1'
        gt_path = [seq['mot_path'] for seq in dataset_res['dataset']['sequences'] if seq['name'] == seq_name][0] / 'gt/gt.txt'
        ...
        return cls(mot_img_path, gt_path, det_path, track_res_path, show_detections, show_gt, show_track_res, show_all_tracks)
