...
from sahi_tracking_tuning.tracker_tuning.param_spaces import sort_experiment_config, oc_sort_experiment_config, viou_experiment_config
...
def sahi_tracking_tune_runner(config):
    ...
    eval_results = run_experiment_framework(config['tracking_dataset_dict'], config['detection_params_dict'],
                                            experiment_dict, model_path = config["model_path"],
    ...
    sequences_small_objects = [
        ...
        #"MOT-1656799357_zoex_1659829_1660035", # Remoived due to Camera Movement
    ]
...
if __name__ == "__main__":
    ...
    detection_params_path = args.detection_params_path.resolve()
    assert detection_params_path, f"Sahi predictions params {detection_params_path} does not exist"
    with open(detection_params_path) as json_file:
        detection_params_dict = json.load(json_file)
